/*
 * CKDBDataNodeLoader.cpp - this file implements the class that has been
 *                          created to handle the asynchronous loading of the
 *                          database-based CKDataNode structures that are in
 *                          use applications. The reason for this class
 *                          is because there are times when the loading of a
 ...
 *                          things are unavailable while we load this one
 *                          lengthy data set.
 *
 *                          But no longer.
 *
 ...
 *                          step and not on any others.
 *
 * $Id: CKDBDataNodeLoader.cpp,v 1.1 2008/01/17 16:26:27 drbob Exp $
 */
 ...
 * around.
 */
CKDBDataNodeLoader::CKDBDataNodeLoader( CKDBDataNodeLoader & anOther ) :
	CKFWThread(),
	mNode(NULL),
	mName()
{
 ...
 */
CKDBDataNodeLoader & CKDBDataNodeLoader::operator=( CKDBDataNodeLoader & anOther )
{
 ...
 * pointer to an CKDBDataNode and fires off a new thread that
 ...
 */
bool CKDBDataNodeLoader::doLoad( CKDBDataNode *aNode, const CKString & aName )
{
	bool		error = false;
	...
	if (!error) {
		if (aNode == NULL) {
			error = true;
			std::cerr << "CKDBDataNodeLoader::doLoad(CKDBDataNode *)"
	...
	CKDBDataNodeLoader	*exec = NULL;
	if (!error) {
		try {
			exec = new CKDBDataNodeLoader();
			if (exec == NULL) {
				error = true;
				std::cerr << "CKDBDataNodeLoader::doLoad(CKDBDataNode *)"
 ...
 * cDone.
 */
int CKDBDataNodeLoader::process()
{
	/*
	...
	if (mNode == NULL) {
		...
		msg << "CKDBDataNodeLoader::process() - the CKDBDataNode is NULL and "
		...
		try {
			...
			std::ostringstream	msg;
			msg << "CKDBDataNodeLoader::process() - while trying to process the "
			...
			msg << "CKDBDataNodeLoader::process() - while trying to process the "
				"load an SAException was thrown: " << excep;
			throw CKException(__FILE__, __LINE__, msg.str());
		}
	}
 ...
 */
int CKDBDataNodeLoader::terminate()
{
	/*
 ...
 ********************************************************/
/*
 * This method checks to see if the two CKDBDataNodeLoaders are
 ...
 */
bool CKDBDataNodeLoader::operator==( const CKDBDataNodeLoader & anOther ) const
{
	bool		equal = true;
 ...
 * a CKString.
 */
CKString CKDBDataNodeLoader::toString()
{
	CKString		retval = "";

	retval += "<CKDBDataNodeLoader>";
 ...
 * he is.
 */
CKDBDataNodeLoader::CKDBDataNodeLoader() :
	CKFWThread(),
	mNode(NULL),
	mName()
{
	// set the thread tag for this guy so I can tell them apart
	setTag("CKDBDataNodeLoader");
}
 ...
 * directly. There is no reason to create or delete a CKDBDataNodeLoader
