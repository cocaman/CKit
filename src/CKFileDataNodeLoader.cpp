/*
 * CKFileDataNodeLoader.cpp - this file implements the class that has been
 *                            created to handle the asynchronous loading of the
 *                            file-based CKDataNode structures that are in
 *                            use in applications. The reason for this
 *                            class is becasue there are times when the loading
 *                            of a group will be very time-consuming but the
 *                            group may not be needed for some time.
 ...
 *                            when the analytic is requested we see if the
 *                            loading is complete. If not, then we hold off on
 *                            that request and not on any others.
 *
 * $Id: CKFileDataNodeLoader.cpp,v 1.1 2008/01/17 16:27:26 drbob Exp $
 */
 ...
 * around.
 */
CKFileDataNodeLoader::CKFileDataNodeLoader( CKFileDataNodeLoader & anOther ) :
	CKFWThread(),
	mNode(NULL),
	mName()
{
 ...
 */
CKFileDataNodeLoader & CKFileDataNodeLoader::operator=( CKFileDataNodeLoader & anOther )
{
 ...
 */
bool CKFileDataNodeLoader::doLoad( CKFileDataNode *aNode, const CKString & aName )
{
	bool		error = false;
	...
	if (!error) {
		if (aNode == NULL) {
			error = true;
			std::cerr << "CKFileDataNodeLoader::doLoad(CKFileDataNode *)"
	...
	CKFileDataNodeLoader	*exec = NULL;
	if (!error) {
		try {
			exec = new CKFileDataNodeLoader();
			if (exec == NULL) {
				error = true;
				std::cerr << "CKFileDataNodeLoader::doLoad(CKFileDataNode *)"
 ...
 * cDone.
 */
int CKFileDataNodeLoader::process()
{
	/*
	...
	if (mNode == NULL) {
		...
		msg << "CKFileDataNodeLoader::process() - the CKFileDataNode is NULL and "
		...
		try {
			...
			std::ostringstream	msg;
			msg << "CKFileDataNodeLoader::process() - while trying to process the "
 ...
 */
int CKFileDataNodeLoader::terminate()
{
	/*
 ...
 ********************************************************/
/*
 * This method checks to see if the two CKFileDataNodeLoader are
 ...
 */
bool CKFileDataNodeLoader::operator==( const CKFileDataNodeLoader & anOther ) const
{
	bool		equal = true;
 ...
 * a CKString.
 */
CKString CKFileDataNodeLoader::toString()
{
	CKString		retval = "";

	retval += "<CKFileDataNodeLoader>";
 ...
 * he is.
 */
CKFileDataNodeLoader::CKFileDataNodeLoader() :
	CKFWThread(),
	mNode(NULL),
	mName()
{
	// set the thread tag for this guy so I can tell them apart
	setTag("CKFileDataNodeLoader");
}
 ...
 * directly. There is no reason to create or delete a CKFileDataNodeLoader
 * object - ever! The static method doLoad() creates one, and the
 * terminate() method takes care of cleaning things up when the work
 * that this thread was meant to do is all done. So... don't go
 * around creating and destroying these guys - the static method
 * does all the work.
 */
CKFileDataNodeLoader::~CKFileDataNodeLoader()
{
