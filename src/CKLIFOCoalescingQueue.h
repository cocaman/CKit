/*
 * CKLIFOCoalescingQueue.h - this file defines a template class that is nice
 *                           coalescing last-in-first-out queue of something.
 ...
 *
 * $Id: CKLIFOCoalescingQueue.h,v 1.3 2008/04/29 20:36:36 drbob Exp $
 */
#ifndef __CKLIFOCOALESCINGQUEUE_H
#define __CKLIFOCOALESCINGQUEUE_H
...
template <class K, class T> class CKLIFOCoalescingQueue;
 ...
 */
#define	CKLIFOCOALESCINGQUEUE_DEFAULT_STARTING_SIZE		8
/*
 ...
 * growing room.
 */
#define	CKLIFOCOALESCINGQUEUE_DEFAULT_INCREMENT_SIZE		16
 ...
 */
template <class K, class T> class CKLIFOCoalescingQueueNotEmptyTest :
	public ICKFWConditionalSpuriousTest
{
	public:
		CKLIFOCoalescingQueueNotEmptyTest( CKLIFOCoalescingQueue<K,T> *aQueue ) :
			mQueuePtr(aQueue)
		{
		}

		virtual ~CKLIFOCoalescingQueueNotEmptyTest()
		{
			mQueuePtr = NULL;
		}
	...
	private:
		CKLIFOCoalescingQueue<K,T>	*mQueuePtr;
};
...
template <class K, class T> class CKLIFOCoalescingQueue
{
	public :
		 ...
		 * just the same.
		 */
		CKLIFOCoalescingQueue( int anInitialCapacity = CKLIFOCOALESCINGQUEUE_DEFAULT_STARTING_SIZE,
							   int aResizeAmount = CKLIFOCOALESCINGQUEUE_DEFAULT_INCREMENT_SIZE ) :
			mElements(),
		 ...
		 * around.
		 */
		CKLIFOCoalescingQueue( CKLIFOCoalescingQueue<K,T> & anOther ) :
			mElements(),
			mKeys(),
			mMutex(),
			mConditional(mMutex)
		...
		CKLIFOCoalescingQueue( const CKLIFOCoalescingQueue<K,T> & anOther ) :
			mElements(),
			mKeys(),
			mMutex(),
			mConditional(mMutex)
		 ...
		 */
		CKLIFOCoalescingQueue<K,T> & operator=( const CKLIFOCoalescingQueue<K,T> & anOther )
		{
			return operator=((CKLIFOCoalescingQueue<K,T> &)anOther);
		}


		CKLIFOCoalescingQueue<K,T> & operator=( CKLIFOCoalescingQueue<K,T> & anOther )
		{
		...
		T pop()
		{
			...
			if (mKeys.empty()) {
				...
				msg << "CKLIFOCoalescingQueue<K,T>::pop() - there are no elements in this queue "
		...
		CKVector<T> pop( int aNumber )
		{
			...
			if (mKeys.empty()) {
				std::ostringstream	msg;
				msg << "CKLIFOCoalescingQueue<K,T>::pop(int) - there are no elements "
		...
		T popSomething()
		{
			...
			CKLIFOCoalescingQueueNotEmptyTest<K,T>	tst(this);
		 ...
		 ********************************************************/
		/*
		 * This method checks to see if the two CKLIFOCoalescingQueues are equal to one
		 ...
		 */
		bool operator==( CKLIFOCoalescingQueue<K,T> & anOther )
		{
			bool		equal = true;
		...
		bool operator==( const CKLIFOCoalescingQueue<K,T> & anOther )
		{
			return operator==((CKLIFOCoalescingQueue<K,T> &)anOther);
		}


		bool operator==( CKLIFOCoalescingQueue<K,T> & anOther ) const
		{
			return ((CKLIFOCoalescingQueue<K,T> *)this)->operator==(anOther);
		}


		bool operator==( const CKLIFOCoalescingQueue<K,T> & anOther ) const
		{
			return ((CKLIFOCoalescingQueue<K,T> *)this)->operator==((CKLIFOCoalescingQueue<K,T> &)anOther);
		}


		/*
		 * This method checks to see if the two CKLIFOCoalescingQueues are not equal to
	...
	private:
		 ...
		 * CKLIFOCoalescingQueue. It's simple, but it's very effective as
		 * I can put all the key,values pushed onto this queue here and
		 * then use the 'keys' LIFOQueue to preserve the order.
		 */
		std::map<K,T>	mElements;
		/*
		 * This is the "LIFO" nature of the queue as this guy will be
...
#endif	// __CKLIFOCOALESCINGQUEUE_H
