...
template <class T> class CKLIFOQueue
{
	public :
		...
		void push( const CKVector<T> & aVector )
		{
			...
			if (mElements == NULL) {
				std::ostringstream	msg;
				msg << "CKLIFOQueue<T>::push(CKVector<T> &) - the storage for "
		...
		T pop()
		{
			...
			if (mElements == NULL) {
				std::ostringstream	msg;
				msg << "CKLIFOQueue<T>::pop() - the storage for this queue "
					"is NULL and that is a data corruption problem that needs to "
					"be looked into as soon as possible. This should never happen.";
				throw CKException(__FILE__, __LINE__, msg.str());
			}
		...
		CKVector<T> pop( int aNumber )
		{
			...
			if (mElements == NULL) {
				std::ostringstream	msg;
				msg << "CKLIFOQueue<T>::pop(int) - the storage for this queue "
					"is NULL and that is a data corruption problem that needs to "
					"be looked into as soon as possible. This should never happen.";
				throw CKException(__FILE__, __LINE__, msg.str());
			}
			if (mSize == 0) {
				std::ostringstream	msg;
				msg << "CKLIFOQueue<T>::pop(int) - there are no elements in this queue "
					"to return. Please use the size() method to verify that there "
					"is something to get.";
				throw CKException(__FILE__, __LINE__, msg.str());
			}
